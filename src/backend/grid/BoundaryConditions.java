...
public interface BoundaryConditions {
    ...
    BoundaryConditions FINITEBOUNDS = (grid, input) -> new CoordinateTuple(
            IntStream.range(0, input.dimension())
                    .map(i -> Math.min(Math.max(input.get(i), grid.getRectangularBounds().getMax(i)), grid.getRectangularBounds().getMin(i))).boxed()
                    .collect(Collectors.toList()));
    BoundaryConditions TOROIDALBOUNDS = (grid, input) -> new CoordinateTuple(
            IntStream.range(0, input.dimension())
                    .map(i -> {

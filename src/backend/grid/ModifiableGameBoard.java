...
public class ModifiableGameBoard extends ModifiableVoogaObject<ModifiableGameBoard> implements GameBoard, GameBoardBuilder {
	...
	public ModifiableGameBoard copy() {
		return new ModifiableGameBoard(getName(), getTemplateCell(), gameBoard, getBoundsHandler(), getDescription(),
				getImgPath());
	}
	...
	public GridBounds getRectangularBounds() {
		return new GridBounds(findMinMax(
				gameBoard.keySet().stream().map(CoordinateTuple::convertToRectangular).collect(Collectors.toList())));
	}

...
public class ActiveAbility<T extends GameObject> extends GameObjectImpl {
    ...
    public static final ActiveAbility<UnitInstance> BLIND = new ActiveAbility<>("Blind", (user, target, game) -> target.addOffenseModifier(InteractionModifier.BLINDED), GridPattern.getNeighborPattern(3), "Gives a unit the Blinded modifier", "Helen_Keller.png");
    public static final ActiveAbility<UnitInstance> SILENCE = new ActiveAbility<>("Silence", (user, target, game) -> Stream.of(target.getOffenseModifiers(), target.getDefenseModifiers(), target.getPassives()).forEach(Collection::clear), GridPattern.getNeighborPattern(3), "Removes all offensive, defensive, and passive modifiers from a unit", "Silencer.png");

...
public class ConditionVBoxFactory {
	...
	private VBox createTurnActionsVBox() {
		...
		for (Actionable act : myController.getAuthoringGameState().getAvailableTurnActions()) {
			box.getChildren().add((new TurnActionBox(act.getName(), myController, myClickHandler)).getNode());
		}

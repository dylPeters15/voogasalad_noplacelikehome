...
public class EntityMovePointPage extends BaseWizardPage {
	 ...
	 *
	 * @param gameState      the AuthoringGameState that the page will use to populate the
	 *                       different terrains
	...
	public Map<VoogaEntity, Integer> getEntityMovePoints() {
		return rowToEntity.keySet().stream().collect(Collectors.toMap(row -> rowToEntity.get(row), IntegerInputRow::getValue));
	}
	...
	private void initialize(String category) {
		...
		defaultVal.setOnAction(event -> rowToEntity.keySet().forEach(row -> row.setValue(defaultVal.getValue())));
